 ...
 * 
 * @author 		Ahmad luky ramdan
 * @copyright   2015
 * @link        https://github.com/achluky
 * 
*/
...
class Ref_agama extends CI_Controller {
	...
	public function r_agama()
	{
		...
		$data['error_code'] = $temp_rec['error_code'];
		$data['error_desc'] = $temp_rec['error_desc'];
		$data['site_title'] = 'Daftar Agama';
		$data['title_page'] = 'Daftar Agama';
		$data['assign_js'] = 'js/agama_dt.js';
		$data['assign_modal'] = '';
		tampil('ref_agama_view',$data);
	}

	public function test_dt()
	{
		$this->load->library('dt_feeder');
		$this->dt_feeder->select('id_kk, nm_kk')->from($this->table);
		echo $this->dt_feeder->generate();
	}

	public function jsonAgama()
	{
		$temp_colum = array(0 => 'id_agama',1 => 'nm_agama');
		...
		if (!empty($sSearch)) {
			$temp_filter = "nm_agama like '%".$sSearch."%'";
			$temp_rec = $this->feeder->getrset($this->session->userdata('token'),
